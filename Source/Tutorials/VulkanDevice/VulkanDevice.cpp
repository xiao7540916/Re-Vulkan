...
namespace Horizon
{
	...
	void Device::DebugMarker::Begin(VkCommandBuffer cb, const char* pMarkerName, const Vector4& color)
	{
		if (initialized)
		{
	...
	void Device::DebugMarker::Insert(VkCommandBuffer cb, const char* pMarkerName, const Vector4& color)
	{
		if (initialized)
		{
			VkDebugMarkerMarkerInfoEXT markerInfo = {};
	...
	Device::Device(Instance* instance, VkPhysicalDevice gpu)
		: mInstance(instance)
	...
	void Device::Init(const DeviceInitInfo& info)
	{
		...
		CreteaLogicalDevice_Internal(info);
	...
	void Device::CreteaLogicalDevice_Internal(const DeviceInitInfo& info)
	{
		...
		const auto& requiredDeviceExtensions = info.deviceExtensions.GetData();
		...
		mGpuFeatures.pNext = info.physicalDeviceFeatures.GetEntry();

		vkGetPhysicalDeviceFeatures2(mGpu, &mGpuFeatures);

		mEnabledValidationLayers = mInstance->GetEnabledInstanceLayers();
	...
	void Device::CreateMemoryAllocator_Internal()
	{
		...
		if (IsDeviceExtensionEnabled(VK_KHR_GET_MEMORY_REQUIREMENTS_2_EXTENSION_NAME) && IsDeviceExtensionEnabled(VK_KHR_DEDICATED_ALLOCATION_EXTENSION_NAME))
		{
			...
			PFN_vkGetBufferMemoryRequirements2KHR getBufferMemoryRequirements2KHR = reinterpret_cast<PFN_vkGetBufferMemoryRequirements2KHR>(vkGetInstanceProcAddr(mInstance->GetHandle(), "vkGetBufferMemoryRequirements2KHR"));
			PFN_vkGetImageMemoryRequirements2KHR getImageMemoryRequirements2KHR = reinterpret_cast<PFN_vkGetImageMemoryRequirements2KHR>(vkGetInstanceProcAddr(mInstance->GetHandle(), "vkGetImageMemoryRequirements2KHR"));
